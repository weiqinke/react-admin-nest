...
import { changeUserStatus } from "@/api/caravan/User";
import { ExclamationCircleOutlined } from "@ant-design/icons";
import { findallarticle, onearticlebody } from "@/api/caravan/Articles";
...
const ArticleList: FC = () => {
  ...
  const columns: any = [
    ...
    },
    {
      ...
      render: (item: any) => {
        return (
          <div>
            <Button size="small" type="primary" onClick={() => seeArticle(item)} style={{ marginRight: "10px" }}>
              查看
            </Button>
            <Button size="small" type="primary" onClick={() => deleteAtricle(item)}>
              删除
            </Button>
          </div>
        );
  ...
  return (
    <div className="users-list-page">
      ...
      <MessageModal visible={tipvisible} pendingCallback={tipcallback} content={tipContent} />
    </div>
  );
};
